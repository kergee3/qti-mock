...
class PromptBuilder:
    ...
    RUBY_INSTRUCTION = """
## ルビ（ふりがな）の付け方
タイトル（title）、問題文（question_text）、模範解答（model_answer）の全ての漢字にルビを付けてください。
形式: {漢字|よみがな}
...
8. 【重要】問題文（question_text）の末尾に、必ず文字数制限を「〇〇字から〇〇字で説明しなさい。」または「〇〇字から〇〇字で書きなさい。」のような形式で記述してください
{ruby_instruction}"""
...
9. 【重要】問題文（question_text）の末尾に、必ず文字数制限を「〇〇字から〇〇字で説明しなさい。」または「〇〇字から〇〇字で書きなさい。」のような形式で記述してください。各問題の指定文字数は問題一覧の「字数制限」列を参照してください
{ruby_instruction}"""
