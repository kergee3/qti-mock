...
class BlobUploader:
    ...
    def upload_file(self, file_path: Path, blob_path: str, max_retries: int = 3, retry_delay: float = 2.0) -> dict:
        ...
        Args:
            ...
            blob_path: Blob上のパス（例: ai-text/social_31_xxx/file.xml）
    ...
    def upload_directory(self, dir_path: Path, blob_prefix: str = "ai-text", update_summary: bool = True) -> list[dict]:
        """
        ディレクトリ内の全ファイルをアップロード

        Args:
            dir_path: アップロードするディレクトリのパス
            blob_prefix: Blob上のプレフィックス（デフォルト: ai-text）
