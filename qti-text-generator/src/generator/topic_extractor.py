...
与えられた学習指導要領の解説テキストから、記述式問題を作成できる具体的なトピック（テーマ）を抽出してください。

## ルール
1. 各トピックは記述式問題を作成できる具体的なテーマにしてください
2. 思考力・判断力・表現力を問える内容を優先してください
3. 単純な知識確認ではなく、理由や仕組みを説明できるトピックを選んでください
4. 抽象的すぎるトピックは避け、具体的な概念・制度・事象を抽出してください
5. 重複や類似するトピックは避けてください
6. 小学生が理解できる内容に限定してください
...
```"""
    ...
    def extract(self, commentary: str, max_topics: int = 10) -> list[str]:
        ...
        user_prompt = f"""以下の学習指導要領の解説テキストから、記述式問題を作成できる具体的なトピックを{max_topics}個以内で抽出してください。

## 重要
- 「なぜ〜か」「どのように〜か」「〜の意義」など、思考力を問えるトピックを選んでください
- 単純な事実確認ではなく、説明・論述が必要なテーマを選んでください

## 解説テキスト
{commentary}

上記の解説テキストから、記述式問題を作成できる具体的なトピックをJSON形式で出力してください。"""
...
if __name__ == "__main__":
    ...
    print("=== Extracting Topics for Text Questions ===\n")

    try:
        extractor = TopicExtractor()
        topics = extractor.extract(sample_commentary, max_topics=5)
